  ...
  getAllEvents(username: any) {
    ...
    forkJoin([holidayEvents$,taskEvents$]).subscribe(
      (results) => {
        ...
        const allEvents = [
          ...holidayEvents.map((event) => {
            const isSameId = event.idEntreprise == this.username;
            console.log(event.idEntreprise+"loblob ",this.username+"labaleb")

            return {
              id: event.id.toString(),
              color: isSameId ? 'blue' : 'red',
              title: isSameId ? event.subject : '',
              description: isSameId ? event.meetingLink : '',
              status: isSameId ? event.meetingLink : '',
              start: new Date(event.dateTime),
              end: new Date(event.dateTime),
              editable: true,
              droppable: true,
          ...
          ...taskEvents.map((task) => {
            const isSameId = task.idEntreprise == this.username;

            return {
              id: task.id.toString(),
              color: isSameId ? 'gray' : 'gray',
              title: isSameId ? task.meetingLink : '',
              description: isSameId ? task.meetingLink : '',
              status: isSameId ? task.meetingLink : '',
              start: new Date(task.dateTime),
              end: new Date(task.dateTime),
              editable: true,
              droppable: true,
