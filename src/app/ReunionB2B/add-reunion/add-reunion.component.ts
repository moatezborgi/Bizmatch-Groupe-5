...
import {ReunionStatus} from "../../Models/Reunion";
...
export class AddReunionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.Reunion.idEntreprise = this.username; // Assure-toi que Reunion est correctement initialisé
   ...
   // reunion.dateTime = new DatePipe('en-US').transform(reunion.dateTime, 'yyyy-MM-ddTHH:mm:ss');
    ...
    this.reunionService.addReunion(reunion).subscribe(
      () => {
        ...
        this.route.navigate(['listReunion/entreprise/1'])
      },
      (error) => {
  ...
  updateMeetLink() {
    this.Reunion.meetingLink = `${this.meetLinkPart1}${this.part2Addition}-${this.part3Addition}-${this.part4Addition}`;
  }
